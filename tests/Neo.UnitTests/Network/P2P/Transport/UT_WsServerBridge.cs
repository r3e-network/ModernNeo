...
//
// UT_WsServerBridge.cs file belongs to the neo project and is free
...
{
    [TestClass]
    public class UT_WsServerBridge
    {
        ...
        private sealed class FakeWsConnection : IWsConnection
        {
#nullable enable
...
#nullable disable
        ...
        [TestMethod]
        [TestCategory("Bridge")]
        public async Task WsServerConnection_Forwards_To_Target()
        {
            using var system = ActorSystem.Create("test");
            var tcs = new TaskCompletionSource<byte[]>(TaskCreationOptions.RunContinuationsAsynchronously);
            var target = system.ActorOf(Props.Create(() => new TestTarget(tcs)));

            var bridge = system.ActorOf(Props.Create(() => new WsServerConnection()));
            bridge.Tell(new Neo.P2P.Abstractions.BridgeBind(target));

            var fake = new FakeWsConnection();
            bridge.Tell(new WsServerConnection.Start(fake));
            ...
            Assert.AreEqual(tcs.Task, received, "Did not receive Tcp.Received on target in time");
