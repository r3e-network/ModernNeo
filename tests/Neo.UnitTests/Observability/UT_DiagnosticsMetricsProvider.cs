...
//
// UT_DiagnosticsMetricsProvider.cs file belongs to the neo project and is free
...
{
    [TestClass]
    public class UT_DiagnosticsMetricsProvider
    {
        [TestMethod]
        public void TestCreateProvider()
        {
            using var provider = new DiagnosticsMetricsProvider("TestMeter", "1.0.0");
            Assert.IsNotNull(provider);
            Assert.AreEqual("TestMeter", provider.MeterName);
        }

        [TestMethod]
        public void TestCreateProviderWithDefaults()
        {
            using var provider = new DiagnosticsMetricsProvider();
            Assert.IsNotNull(provider);
            Assert.AreEqual("Neo", provider.MeterName);
        }
        ...
        {
            var provider = new DiagnosticsMetricsProvider();
