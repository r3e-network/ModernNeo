...
| IWitnessConditionData     | 见证条件纯数据契约，无 VM 依赖           | WitnessCondition     |
| ITransactionAttributeData | 交易属性纯数据契约，无 DataCache 依赖    | TransactionAttribute |
...
| WitnessCondition     | TypeValue             | 条件类型字节值 |
| TransactionAttribute | TypeValue             | 属性类型字节值 |
