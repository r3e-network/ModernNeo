...
//
// GetBlockRpcMethod.cs file belongs to the neo project and is free
...
public sealed class GetBlockRpcMethod : IRpcMethod
{
    private readonly NeoSystemNode _node;

    public string Name => "getblock";

    public GetBlockRpcMethod(NeoSystemNode node)
    {
    ...
    public Task<JToken?> ProcessAsync(JArray? parameters)
    {
        ...
        if (!verbose)
            return Task.FromResult<JToken?>(new JString(block.ToArray().ToHexString()));

        var json = block.ToJson(_node.System.Settings);
        AddBlockMetadata(json, block.Index);
        return Task.FromResult<JToken?>(json);
    }

    private Neo.Network.P2P.Payloads.Block? ResolveBlock(JToken token)
    {
        if (RpcParameterParser.TryGetUInt32(new JArray(token), 0, out var index))
            return NativeContract.Ledger.GetBlock(_node.System.StoreView, index);

        var text = token.AsString();
        if (UInt256.TryParse(text, out var hash))
            return NativeContract.Ledger.GetBlock(_node.System.StoreView, hash);
