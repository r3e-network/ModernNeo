...
namespace Neo.Orleans.Dbft.Consensus
{
    public sealed partial class ConsensusContext : IDisposable, ISerializable
    {
        ...
        public void Reset(byte viewNumber, int? myIndexOverride = null)
        {
            if (viewNumber == 0)
            {
                Snapshot?.Dispose();
                ...
                if (myIndexOverride.HasValue && myIndexOverride.Value >= 0 && myIndexOverride.Value < Validators.Length)
                {
                    MyIndex = myIndexOverride.Value;
                    _myPublicKey = Validators[MyIndex];
                }
