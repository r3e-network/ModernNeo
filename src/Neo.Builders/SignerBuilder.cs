...
{
    public sealed class SignerBuilder : ISignerBuilder<Signer>
    {
        private UInt160 _account;
        ...
        ISignerBuilder<Signer> ISignerBuilder<Signer>.Account(byte[] scriptHash)
        {
            _account = new UInt160(scriptHash);
            return this;
        }

        ISignerBuilder<Signer> ISignerBuilder<Signer>.Scopes(byte scopes)
        {
            _scopes = (WitnessScope)scopes;
            return this;
        }

        ISignerBuilder<Signer> ISignerBuilder<Signer>.AllowContract(byte[] contractHash)
        {
            AllowContract(new UInt160(contractHash));
            return this;
        }

        ISignerBuilder<Signer> ISignerBuilder<Signer>.AllowGroup(byte[] publicKey)
        {
            AllowGroup(ECPoint.DecodePoint(publicKey, ECCurve.Secp256r1));
            return this;
        }
    }
}
